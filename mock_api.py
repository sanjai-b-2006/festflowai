...
def setup_database():
    ...
    if not os.path.exists(USERS_FILE) or os.path.getsize(USERS_FILE) == 0:
        save_data(USERS_FILE, [
            {"username": "treasurer", "password": "pw", "name": "Alex Ray", "role": "treasurer"},
            {"username": "team_lead", "password": "pw", "name": "Leo Valdez", "role": "team_lead"},
            {"username": "student1", "password": "pw", "name": "Ben Carter", "role": "student", "upi_id": "ben.carter@upi"},
        ])
...
def get_expenses_for_user(username):
    ...
    user_expenses = list(filter(lambda e: e['user'] == username, all_expenses))
...
def get_pending_requests(user_role):
    ...
    if user_role == "treasurer":
        # Treasurer needs to see items pending their approval AND items ready for reimbursement.
...
def reimburse_expense(expense_id, approver_user):
    expenses = load_data(EXPENSES_FILE)
    for expense in expenses:
        if expense['id'] == expense_id and expense['status'] == 'Approved':
            expense['status'] = 'Reimbursed'
            expense['reimbursed_at'] = datetime.datetime.now()
            expense['transaction_id'] = f"UpiTXN{int(datetime.datetime.now().timestamp())}"
            save_data(EXPENSES_FILE, expenses)
            log_activity(approver_user['name'], f"reimbursed expense #{expense_id}.")
            return True
    return False